                                    ...
                                    <tbody>
                                        ...
                                        if (!empty($allCards)) {
                                            foreach ($allCards as $card) {
                                                if (!empty($cardDetails[$i]['id'])) {
                                                    ?>
                                                    <tr>
                                                        <?php if ($this->request->session()->read('Auth.User.type') == 1) { ?>
                                                            <td><input type="checkbox" name="chk[]" class="chk_card" value="<?php echo $card['card_token']; ?>"></td>
                                                        <?php } ?>
                                                        <td><?php echo $card['card_brand']; ?></td>
                                                        <td><?php echo $card['card_no']; ?></td>
                                                        <td><?php echo $card['name_on_card']; ?></td>
                                                        <td style="text-align: center;"><?= $card['expiry_month'] . '-' . $card['expiry_year']; ?></td>
                                                        <td style="text-align: center;"><?= $this->Custom->cardStatus($cardDetails[$i]['is_approve']); ?></td>
                                                        <?php if ($this->request->session()->read('Auth.User.type') == 1) { ?>
                                                            <td style="text-align: center;">
                                                                <?php if ($cardDetails[$i]['is_approve'] == 0 || $cardDetails[$i]['is_approve'] == 2) { ?>
                                                                    <a href="<?php echo HTTP_ROOT . 'admin/users/cardapprove/' . $card['card_token']; ?>">
                                                                        <button class="btn btn-sm btn-primary" type="button">
                                                                            Click to Activate
                                                                        </button>
                                                                    </a>
                                                                <?php } else { ?>
                                                                    <a href="<?php echo HTTP_ROOT . 'admin/users/carddisapprove/' . $card['card_token']; ?>"><button class="btn btn-sm btn-primary btn-color" type="button">
                                                                            Click to De-Activate
                                                                        </button>
                                                                    </a>
                                                                <?php } ?>
                                                                <a href="javascript:;">
                                                                    <button type="button" class="btn btn-sm btn-success" data-toggle="modal" data-target="#myModal-<?= $card['card_token'] ?>">
                                                                        View
                                                                    </button>
                                                                </a>
                                                                ...
                                                                <!--Popup Code-->
                                                                <!-- Modal -->
                                                                <div id="myModal-<?= $card['card_token'] ?>" class="modal fade" role="dialog">
                                                                                ...
                                                                                <div class="row" style=""> 
                                                                                    <table align='center'>
                                                                                        ...
                                                                                        <tr>
                                                                                            ...
                                                                                            <td style="padding: 10px; text-align: left;"> : <?= $card['name_on_card'] ?></td>
                                                                                        </tr>
                                                                                        <!--
                                                                                        <tr>
