...
class UsersController extends AppController {
    ...
    public function retry($transactionId) {
        ...
        if (isset($response->messageCode) && $response->messageCode == 200) {
            $status = $this->Transactions->query()->update()->set(['pending_url_hit' => 1])->where(['transaction_id' => $transactionId])->execute();
    ...
    public function dashboard() {
        ...
        foreach ($getUsers as $getUser) {
            $var1 = "{$key}:{$getUser->id}"; // User_Credentials
            ...
            if ($userCards['status'] == 1) {
                $allCards = array_merge($allCards, $userCards['user_cards']);
            }
        }

        $cardDetails = [];
        foreach ($allCards as $card) {
            $query = $this->Cards->find()->where(['Cards.card_token' => $card['card_token']]);
            if ($query->count() > 0) {
        ...
        $this->set(compact('allCards', 'cardDetails'));
    }
    ...
    public function transactions() {
        ...
        if (isset($data['download_report'])) {
            ...
            ///SetHeading//  
            $objPHPExcel->getActiveSheet()->SetCellValue('A1', "Txn ID");
            $objPHPExcel->getActiveSheet()->SetCellValue('B1', "Transaction Date");
            $objPHPExcel->getActiveSheet()->SetCellValue('C1', "Agent ID");
            $objPHPExcel->getActiveSheet()->SetCellValue('D1', "Agent Name");
            $objPHPExcel->getActiveSheet()->SetCellValue('E1', "Amount");
            $objPHPExcel->getActiveSheet()->SetCellValue('F1', "Card No");
            $objPHPExcel->getActiveSheet()->SetCellValue('G1', "Txn Status");
            $objPHPExcel->getActiveSheet()->SetCellValue('H1', "Indigo Navitor Status");
            //Set Content
            $rowCount = 2;
            foreach ($transactions as $transaction) {
                $objPHPExcel->getActiveSheet()->SetCellValue('A' . $rowCount, $transaction->transaction_id);
                $objPHPExcel->getActiveSheet()->SetCellValue('B' . $rowCount, $transaction->created);
                $objPHPExcel->getActiveSheet()->SetCellValue('C' . $rowCount, $transaction->user->agent_id);
                $objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, $transaction->user->firstname . ' ' . $transaction->user->lastname);
                $objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $transaction->amount);
                $objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $transaction->card_number);
                $objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, $this->Custom->paymentStatus($transaction->payment_status));
                $objPHPExcel->getActiveSheet()->SetCellValue('H' . $rowCount, $this->Custom->navitorStatus($transaction->payment_status, $transaction->navitor_status));
